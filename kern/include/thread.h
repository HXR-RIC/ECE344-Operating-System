...
extern struct thread **thread_array;
extern int child_excode[100];
...
struct thread {
	...
	struct vnode *t_cwd;
        ...
        //build for storing this thread's pid 
        pid_t pid;
...
void thread_wakeup_signal(const void *addr);
/*
...
//used for finding child pid
int search_Child_pid(pid_t pid, struct thread* thread);

//used for check if it is its child
int check_child(pid_t pid, struct thread* thread);

int piderror(pid_t new_pid);
struct thread *find_thread(pid_t new_pid);
void initial_thread(pid_t new_pid);
