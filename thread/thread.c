...
thread_create(const char *name) {
    ...
    if (thread == NULL) {
        kprintf("cannot allocate memory for thread\n");
        return NULL;
    }
    ...
    int i;
    for (i = 0; i < 100; i++)
        thread->child_pid[i] = 0;

    thread->child_excode = 0;


    P(lock);
    if (start) {
        int i;
        thread_array = kmalloc((MAX_THREAD_NUMBER) * sizeof (struct thread*));
        for (i = 0; i < MAX_THREAD_NUMBER; i++)
            thread_array[i] = kmalloc(sizeof (struct thread));
        start = 0;
    }
    // V(lock);

    // P(lock);
    if (thread_array_count > 0 && thread_array_count < MAX_THREAD_NUMBER) {
        thread_array[thread_array_count] = curthread;
        thread->pid = thread_array_count;
        thread_array_count++;
    }
...
thread_bootstrap(void) {
    ...
    // waitlock = sem_create("waitlock",1);
    /*
...
thread_fork(const char *name,
    ...
    /*link this new thread to our structure*/


    curthread->child_pid[newguy->pid] = newguy->pid;
     ...
     */
    if (ret != NULL) {
        //kprintf("done thread fork with pid %d\n", newguy->pid);
        //kprintf("done thread fork with name %s\n", newguy->t_name);
        // kprintf("done thread fork with parent pid %d\n", newguy->parent_pid);
        *ret = newguy;
    }
...
void
thread_wakeup_signal(const void *addr) {
    int i, result;
